...
class GPT(nn.Module):
    ...
    def configure_optimizers(self, weight_decay, learning_rate, beta1, beta2,device):
        ...
        # Weight decay for 2D parameters (weights), no decay for 1D (biases, layernorms)
...
class GPT_abacus(nn.Module):
    ...
    def forward(self, idx, targets=None):
        ...
        pos_emb = self.transformer.wpe(pos)
        tok_emb = self.transformer.wte(idx)
        abacus_emb = self.transformer.abacus(idx)
...
class GPT_RoPE(nn.Module):
    def __init__(self, config):
        ...
        self.transformer = nn.ModuleDict(dict(
            wte = nn.Embedding(config.vocab_size, config.n_embd),
            h = nn.ModuleList([RoPEBlock(config) for _ in range(config.n_layer)]),
            ln_f = nn.LayerNorm(config.n_embd),
        ))
        self.lm_head = nn.Linear(config.n_embd, config.vocab_size, bias=False)
